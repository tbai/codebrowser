...
exports.configure = function (app) {
    app.param('repository', function (req, res, next, id) {
        db.Repository.findOne({ _id: id }, function (err, repo) {
            if (!err && repo) {
                console.info("Loaded repository param: " + id)
                req.session.repository = id;
                res.locals.repository = id;
                next();
            } else {
                console.warn("Failed to load repository: " + id + " err=" + err);
                next();
            }
        });
    });
...
exports.getCurrentRepositoryId = function (req) {
    return req.session && req.session.repository ? req.session.repository : config.DEFAULT_REPOSITORY_ID;
}
...
function gitPull(repository, callbackFn) {
    ...
    exec(cmd, function (fetchError, fetchOutput) {
        ...
        // rename current branch: 
        // git merge -s recursive -X theirs origin/23s/green
        cmd = cmdStr("git merge -s recursive -X theirs " + repository.remoteBranch);
...
function gitPullAll(callbackFn){
    ...
    db.Repository.find(function (error, repoList) {
        ...
        for (var i = 0; i < repoList.length; i++) {
            exports.gitPull(repoList[i], function (gitError) {
                nResolved++;
